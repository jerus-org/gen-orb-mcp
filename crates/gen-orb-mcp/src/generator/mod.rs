...
mod tests {
    ...
    fn test_generate_with_tools_cargo_toml_includes_gen_orb_mcp_dep() {
        ...
        // Pinned to the exact gen-orb-mcp version that generated this server
        // (crates.io, not a git/branch dependency) so rebuilding an
        // already-generated server can't silently pick up API/behavior
        // changes from a later gen-orb-mcp release.
        let expected_pin = format!("gen-orb-mcp = \"={}\"", env!("CARGO_PKG_VERSION"));
        assert!(
            cargo_toml.contains(&expected_pin),
            "expected exact-pinned gen-orb-mcp dependency entry when has_tools, got:\n{cargo_toml}"
        );
        assert!(
            !cargo_toml.contains("git = \"https://github.com/jerus-org/gen-orb-mcp\""),
            "gen-orb-mcp dependency must not track a git branch"
        );
    }
